...
<form action="" method="POST" class="form-inline" role="form">
	...
	<div class="form-group">
		...
		<select name="tapel" id="inputKelas" class="form-control" required="required">
			...
			foreach($jb as $jb){
			?>
			<option value="<?=$jb['id'];?>"><?=$jb['awal'];?> / <?=$jb['akhir'];?></option>
			<?php } ?>
		</select>
	</div>
...
<div class="col-lg-10 col-lg-offset-1">
	<h3 class="text-center">Grafik <br>Kelas <?=konvert('kelas',$kelas,'nama');?> (<?=konvert('tapel',$tapel,'awal');?>/<?=konvert('tapel',$tapel,'akhir');?>)</h3>
	<canvas id="myChart" heigth="400"></canvas>
</div>
<div class="col-lg-10 col-lg-offset-1">
	<h3 class="text-center">Tabel <br>Kelas <?=konvert('kelas',$kelas,'nama');?> (<?=konvert('tapel',$tapel,'awal');?>/<?=konvert('tapel',$tapel,'akhir');?>)</h3>
